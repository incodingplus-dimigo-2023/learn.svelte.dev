...
export function get_index() {
	const parts = fs.readdirSync(`${con}/tutorial`).filter(is_valid).map(posixify);

	return parts.map((part) => {
		const chapters = fs.readdirSync(`${con}/tutorial/${part}`).filter(is_valid).map(posixify);
		...
		return {
			...
			chapters: chapters.map((chapter) => {
				const exercises = fs
					.readdirSync(`${con}/tutorial/${part}/${chapter}`)
